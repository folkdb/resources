...
export const buildJson = (context, {
  ...
  const eachCategory = (name, parentDir) => tryCatch(
    async () => {
      const currentDir = absolute(name, parentDir);
    ...
    {
      source,
      call: 'eachCategory(name, parentDir)',
      arguments: { name, parentDir },
    },
  );
