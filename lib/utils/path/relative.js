...
export const relative = (pathOrUrl, context = process.cwd()) => {
  ...
  if (whatType(pathOrUrl) !== 'String') {
    throw (new Error(`Expected String but got ${whatType(pathOrUrl)} at ${stringify({ library, source, call, argument: 'pathOrUrl' })}`));
  }
  
  if (whatType(context) !== 'String') {
    throw (new Error(`Expected String but got ${whatType(context)} at ${stringify({ library, source, call, argument: 'context' })}`));
  }
